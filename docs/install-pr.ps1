...
# Usage: irm https://raw.githubusercontent.com/davidfowl/tally/main/docs/install-pr.ps1 | iex; Install-TallyPR <PR_NUMBER>
#
...
$Script:Repo = "davidfowl/tally"
$Script:InstallDir = "$env:LOCALAPPDATA\tally"
...
function Get-WorkflowRunId {
    ...
    $pr = gh api "repos/$Script:Repo/pulls/$PRNumber" | ConvertFrom-Json
    if (-not $pr) {
    ...
    $runs = gh api "repos/$Script:Repo/actions/workflows/pr-build.yml/runs?head_sha=$headSha&status=success" | ConvertFrom-Json

    if (-not $runs.workflow_runs -or $runs.workflow_runs.Count -eq 0) {
        Write-Err "No successful build found for PR #$PRNumber.
Check https://github.com/$Script:Repo/pull/$PRNumber/checks"
    }

    return $runs.workflow_runs[0].id
}

function Install-TallyPR {
    param(
    ...
    # Get workflow run ID
    $RunId = Get-WorkflowRunId -PRNumber $PRNumber
    ...
    try {
        gh run download $RunId -R $Script:Repo --name $ArtifactName
        if ($LASTEXITCODE -ne 0) {
            Write-Err "Failed to download artifact. The build may still be in progress.
Check https://github.com/$Script:Repo/actions/runs/$RunId"
        }
    } finally {
        Pop-Location
    }
    ...
    Move-Item -Path (Join-Path $TempDir "tally.exe") -Destination (Join-Path $Script:InstallDir "tally.exe") -Force
    ...
    & "$Script:InstallDir\tally.exe" version
...
# Export function for use when sourced
Export-ModuleMember -Function Install-TallyPR -ErrorAction SilentlyContinue
